#!/usr/bin/env bash
# Build and deploy the ACM split-brain arbitrator on the hub (rose-fis).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
HUB_KUBECONFIG="${HUB_KUBECONFIG:-$HOME/Downloads/rose-fis-kubeconfig.yaml}"
NS=open-cluster-management
APP_DIR="$ROOT/akka-split-brain-arbitrator"
NAME=akka-split-brain-arbitrator

export KUBECONFIG="$HUB_KUBECONFIG"

echo "==> Applying RBAC + config + service + route"
...
# Point deployment at the ImageStream tag produced by the binary build.
...
# Ensure the running pod is on the newest ImageStream digest (not a stale :latest pull).
...
echo "==> Arbitrator route: https://$ROUTE"
echo "    curl -sk -H 'X-Cluster-Name: cluster1-fis' https://$ROUTE/api/v1/state | jq ."
echo "    curl -sk -H 'X-Cluster-Name: cluster2-fis' https://$ROUTE/api/v1/state | jq ."
